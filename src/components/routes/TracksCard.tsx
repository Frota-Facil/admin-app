...
import { formatDateTime, getDateTimestamp } from "@/utils/date-format";
...
export function TracksCard({ routeId, status, tracks }: TracksCardProps) {
  ...
  const sortedTracks = [...tracks].sort(
    (leftTrack, rightTrack) =>
      getTrackTimestamp(leftTrack.capturedAt) -
      getTrackTimestamp(rightTrack.capturedAt),
  );
  ...
  return (
    <div className="space-y-4 p-5">
      <article className="overflow-hidden rounded-lg border border-slate-200 bg-slate-50">
        <div className="h-90 overflow-hidden bg-slate-100 md:h-130">
...
function TrackInfo({ label, value }: TrackInfoProps) {
  return (
    <div className="min-w-0">
      ...
      <dd className="mt-1 wrap-break-word font-semibold text-slate-950">
        {value}
      </dd>
    </div>
  );
...
function getTrackTimestamp(value?: Date | string | null) {
  return getDateTimestamp(value) ?? 0;
}
